...
const ManageAllEmails: React.FC = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const [responseLength, setReponseLength] = useState<number>(0);
  const [allEmailEnquiry, setAllEmailEnquiry] = useState<[IContactEmails]>();
  const [filteredOutput, setFilterdOutput] = useState<
    IContactEmails[] | undefined
  >([]);
  ...
  useEffect(() => {
    const getAllEmailEnquiries = async () => {
      ...
      try {
        ...
        if (response) {
          setAllEmailEnquiry(response.data);
    ...
    getAllEmailEnquiries();
  }, []);

  const showAndHideAllData = () => {
    setEnquiryPerPage(responseLength);
  };
  ...
  const currentPageData = allEmailEnquiry?.slice(
    indexOfFirstEnquiry,
    indexOfLastEnquiry
  );
  ...
  const getSearchInput: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    const result = allEmailEnquiry?.filter((enquiry) => {
      const searchOutput = enquiry.userName.toLowerCase();
  ...
  const pageNumber = [];
  for (let i = 1; i <= Math.ceil(responseLength / enquiryPerPage); i++) {
    pageNumber.push(i);
  }
...
export default ManageAllEmails;
